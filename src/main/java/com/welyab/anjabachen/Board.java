...
public class Board {
	 ...
	 */
	private static final int SQUARES_COUNT = BOARD_SIZE * BOARD_SIZE;
	...
	private void move(PieceMovement movements, MovementTarget target) {
		MoveLog moveLog = null;
		Square originSquare = getSquare(movements.getRow(), movements.getColumn());
		...
		if (isPawnPromotion) {
			...
			targets.add(
				new MovementTarget(
					...
					target.getRow(),
					target.getColumn(),
					POSITIONS[origin.getRow()][target.getRow()]
				)
			);
		} else {
			targets.add(
	...
	private static class MoveLog {
		...
		final MovementTarget target;
		
		final PieceMovement movement;
		
		MoveLog(MovementTarget target, PieceMovement movement) {
			this.target = target;
