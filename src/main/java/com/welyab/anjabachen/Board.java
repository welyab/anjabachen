...
public class Board implements Copiable<Board> {
	...
	public static void main(String[] args) throws Exception {
		var board = new Board("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1");
		var movements = Files.lines(Paths.get("/home/welyab/Desktop/calculatedmoves.txt")).collect(Collectors.toList());
		for (var movePath : movements) {
			String[] paths = movePath.split(" ");
			board.undoAll();
			for (var path : paths) {
				var pgnNotationOrigion = path.substring(0, 2);
				var pgnNotationTarget = path.substring(2, 4);
				board.move(
					Position.of(pgnNotationOrigion.charAt(0), pgnNotationOrigion.charAt(1) - '0'),
					Position.of(pgnNotationTarget.charAt(0), pgnNotationTarget.charAt(1) - '0')
				);
			}
		}
	}
					...
					if (isInsideBoard(targetRow, targetColumn)) {
						...
						if (grid[targetRow][targetColumn] != null) {
							...
							if (targetPiece.getColor().equals(attackerColor)
								...
								if (discovery && !GameConstants.isDoubleCheck(flags)) {
									flags |= GameConstants.DISCOVERY_CHECK;
								} else if (GameConstants.isDoubleCheck(flags)) {
									flags &= ~GameConstants.DISCOVERY_CHECK;
								}
							} else {
