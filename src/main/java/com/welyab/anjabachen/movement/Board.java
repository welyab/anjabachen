...
import com.welyab.anjabachen.ChessError;
...
public class Board implements Copiable<Board> {
	 ...
	 */
	public Board() {
		this(BoardUtils.FEN_INITIAL_POSITION);
	}
	...
	private Board(
			Piece[][] grid,
			GameInfo gameInfo,
			List<MovementLog> movementHistory
	) {
		this.grid = grid;
	...
	private interface SquareVisitor {
		...
		boolean visit(Position targetPosition);
	}
	
	private void walkPiece(
			Position originSquare,
			List<Direction> directions,
			int maxMoveLength,
			SquareVisitor squareVisitor
	) {
		for (int t = 0; t < directions.size(); t++) {
			for (int mLength = 1; mLength <= maxMoveLength; mLength++) {
				Direction directionAdjuster = directions.get(t);
				int targetRow = originSquare.getRow() + mLength * directionAdjuster.rowAdjuster;
				int targetColumn = originSquare.getColumn() + mLength * directionAdjuster.columnAdjuster;
				if (!isInsideBoard(targetRow, targetColumn)) {
					break;
				}
				Position targetPosition = Position.of(targetRow, targetColumn);
				boolean continueWalk = squareVisitor.visit(targetPosition);
				if (!continueWalk) {
					break;
				}
			}
	...
	private List<Position> getAttackers(
		...
		walkPiece(
			squarePosition,
			getDirectionAdjuster(attackerPiece),
			getMaxPieceMovementLength(attackerPiece.getType()),
			target -> {
				Piece targetPiece = getPieceOrNull(target);
				if (targetPiece == null) {
					return true;
				}
	...
	private List<Position> getAttackersFromKing(Position squarePosition, Color attackerColor) {
		return getAttackers(squarePosition, Piece.get(PieceType.KING, attackerColor));
	}
	
	private List<Position> getAttackersFromQueen(Position squarePosition, Color attackerColor) {
		return getAttackers(squarePosition, Piece.get(PieceType.QUEEN, attackerColor));
	}
	
	private List<Position> getAttackersFromRook(Position squarePosition, Color attackerColor) {
		return getAttackers(squarePosition, Piece.get(PieceType.ROOK, attackerColor));
	}
	
	private List<Position> getAttackersFromBishop(Position squarePosition, Color attackerColor) {
		return getAttackers(squarePosition, Piece.get(PieceType.BISHOP, attackerColor));
	}
	...
	private List<Position> getAttackersFromKnight(Position squarePosition, Color attackerColor) {
		return getAttackers(squarePosition, Piece.get(PieceType.KNIGHT, attackerColor));
	}
	...
	public Movement getLastMovement() {
		MovementLog movementLog = movementHistory.get(movementHistory.size() - 1);
		return new Movement(
			movementLog.movementOrigin,
			movementLog.movementTarget
		);
	}
	...
	private List<Direction> getDirectionAdjuster(Piece piece) {
		try {
	...
	private PieceMovement getKingQueenRookBishopKnightMovements(
		...
		List<Direction> directionAdjusters = getDirectionAdjuster(originPiece);
	...
	private int extractKingQueenRookBishopExtraFlags(int flags) {
		...
		for (int t = 0; t < queenMoveTemplate.size(); t++) {
			boolean discovery = BoardUtils.isDiscoveryCheck(flags);
			for (int mLength = 1; mLength <= maxMoveLength; mLength++) {
	 ...
	 */
	PieceMovement getPawnMovements(Position pawnPosition, boolean extractExtraMovementFlags) {
		List<MovementTarget> targets = new ArrayList<>();
		MovementMetadata.Builder pieceMovementMetaBuilder = MovementMetadata.builder();
