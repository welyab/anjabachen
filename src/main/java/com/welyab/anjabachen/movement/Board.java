 ...
 * <strong>ANJABACHEN</strong>. It keeps internally a piece disposition state,
 * also castling
 * ability flags, <i>en passant</i> captures, etc.
 * 
 * <p>
 * The <code>Board</code> class can manage movement generation also to the chess
 * variant
 ...
 * </pre>
 * 
 * The code creates a board with pieces in the initial position, then retrieves
 * the list of
 * available
 ...
 * </pre>
 * 
 * The above code creates a board and retrieve from it a random move for white
 * pieces, then submits
 * that move to the board. The following call to the <code>getMovements</code>
 * will retrieve the
...
public final class Board implements Copyable<Board> {
    ...
    /**
     * Directions for white pawns' single square movement forward (white point
     * of view).
     */
    ...
    /**
     * Directions for black pawns' single square movement forward (black point
     * of view).
     */
    ...
    /**
     * Directions for white pawns' double square movement forward (white point
     * of view).
     */
    ...
    /**
     * Directions for black pawns' double square movement forward (black point
     * of view).
     */
    ...
    /**
     * Walk all squares of the board, calling the given <code>visitor</code>
     * when the square is
     * not empty.
     * 
     ...
     * 
     * @return A value <code>true</code> if the square is empty, or
    ...
    /**
     * Retrieves a copy of the current board state. Changes in the returned
     ...
     * 
     * @throws EmptySquareException If the square in the given position is
     *             empty.
     * 
     * @see #getMovements()
    ...
    /**
     * Retrieves all movements of all pieces of the side that has the turn to
     * move.
     * 
     * <p>
     * This method extract all movement meta information available.
	...
	byte color = state.getSideToMove();
	List<LocalizedPiece> attackers = getAttackers(
	...
	if (check && discoveryCheck && attackers.size() != 2) {
	    System.out.println(origin + " -> " + target);
	    System.out.println(this);
	}

	if (!check && discoveryCheck && attackers.size() == 2) {
	    System.out.println("!check && discoveryCheck && attackers.size() == 2");
	    System.out.println(origin + " -> " + target);
	    System.out.println(this);
	}
	...
	for (int i = 1; i <= 7; i++) {
	    ...
	    if (target != null && target.equals(targetRow, targetColumn)) {
		return false;
	    }
    ...
    /**
     * Evaluates if given position is attacked by any piece of the specified
     * color.
     * 
     * @param position The attacked position.
     * @param attackerColor The attacker color.
     * 
     * @return A value <code>true</code> if the position is attacked, or
    ...
    /**
     * The maximum movement length that a piece may perform during a single
     ...
     * </pre>
     * 
     * The above diagram shows that black rook in c6 is pinned, but a call for
     * this method querying
	 ...
	 * @param pieceCode The piece code of the piece positioned in the
	 *            informed square, or the
	 *            code representative for empty square
	 *            ({@link MovementUtil#EMPTY}).
	 * 
	 * @return Should return an information to the walker if the walking may
	 *         continue, stop an
	 *         direction of stop entire walking. There three constants for
	 *         indicating that
	 *         behavior:
	 * 
	 *         <ul>
    ...
    /**
     * Retrieves the FEN string representative for this board in its current
     * state.
     * 
